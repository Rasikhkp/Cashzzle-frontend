...
const icons = [food, bus, house, hospital, beach, alarm_clock, lounge_chair, fak, ball, book, gift, graph, medal, money, money_bag, shop]
...
const CategoriesChooseIcon = ({ setValue }: any) => {
    ...
    return (
        <div className='relative'>
            <button onClick={() => setOpenChooseIcon(!openChooseIcon)} type='button' className={`${openChooseIcon ? "bg-gray-200" : ""} flex border border-gray-800 rounded-lg items-center justify-center w-[38px] h-[38px]`}>
                <Image
                    src={iconSelected}
                    width={24}
                    height={24}
            ...
            {openChooseIcon && (
                <Card className='p-2 flex flex-wrap gap-2 w-[196px] justify-center absolute right-0 bottom-[130%]'>
                    {icons.map((icon, index) => (
                        <button key={index} onClick={() => selectIcon(icon.src)} type='button' className="flex hover:bg-accent rounded-lg items-center justify-center w-[38px] h-[38px]">
                            <Image
                                src={icon}
                                width={24}
                                height={24}
